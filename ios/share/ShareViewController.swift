...
let frigoligoColor = Color(
  UIColor(red: 0.31372550129999999, green: 0.61960786580000005, blue: 0.7607843876, alpha: 1.0))
...
struct ShareExtView: UIViewControllerRepresentable {
  func buildFlutterEngine() -> FlutterEngine {
    ...
    guard engine.run(withEntrypoint: "mainNativeShare") else {
    ...
    FPPPackageInfoPlusPlugin.register(
      with: engine.registrar(forPlugin: "FPPPackageInfoPlusPlugin")!)
    PathProviderPlugin.register(with: engine.registrar(forPlugin: "PathProviderPlugin")!)
    SharedPreferenceAppGroupPlugin.register(
      with: engine.registrar(forPlugin: "SharedPreferenceAppGroupPlugin")!)
    ...
    Sqlite3FlutterLibsPlugin.register(
      with: engine.registrar(forPlugin: "Sqlite3FlutterLibsPlugin")!)
...
class ShareViewController: UIViewController {
  private static let loggerName = "ShareViewController"
  ...
  private func exitExtension(withErrorMessage: String? = nil) {
    if withErrorMessage != nil {
      SystemBridge.logError(withErrorMessage!, logger: ShareViewController.loggerName)
      let alert = UIAlertController(
        title: "Error", message: withErrorMessage, preferredStyle: .alert)
      let action = UIAlertAction(title: "OK", style: .default) { _ in
        self.dismiss(animated: true, completion: nil)
        let error = NSError(
          domain: "net.casimir-lab.frigoligo", code: 0,
          userInfo: [NSLocalizedDescriptionKey: withErrorMessage!])
    ...
    for typeIdentifier in [UTType.url.identifier, UTType.plainText.identifier] {
      ...
      do {
        ...
        SystemBridge.logError(
          "could not extract URL (type: \(typeIdentifier)): \(error)",
          logger: ShareViewController.loggerName)
      }
    }
    ...
    do {
      ...
      if articleId != nil {
        SystemBridge.logInfo(
          "article saved with id \(articleId!)", logger: ShareViewController.loggerName)
      } else {
        SystemBridge.logInfo(
          "the article will be saved with the next successful sync",
          logger: ShareViewController.loggerName)
      }
    } catch {
