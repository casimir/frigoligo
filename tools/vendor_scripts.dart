...
final _scriptsDir = '$_projectRoot/assets/www/scripts';
final _assetsWww = '$_projectRoot/assets/www';
...
void _vendor(WebViewNpmScript script) {
...
void _updatePubspec() {
  ...
  final dirs = _assetDirs(_assetsWww, 'assets/www')..sort();
  final lines = '${dirs.map((d) => '    - $d/').join('\n')}\n';

  final updated = _replaceMarkers(original, begin, end, lines);
  if (updated == null) {
    print('No webview-scripts markers in pubspec.yaml — skipping.');
    return;
  }
...
/// Collects every directory under [absPath] that directly contains at least
/// one non-hidden file, as relative paths from the project root.
...
void _generateManifest(Map<String, String> configContents) {
  ...
  for (final script in webViewCustomScripts) {
    entries.add({
      'name': script.name,
      'path': 'assets/www/scripts/${script.file}',
...
void _updateTestHtml(Map<String, String> configContents) {
  ...
  for (final time in ScriptInjectionTime.values) {
    ...
    final indent = time == ScriptInjectionTime.atDocumentStart ? '    ' : '  ';
    ...
    for (final script in webViewNpmScripts.where(
      ...
      if (script.preScriptFile case final f?) {
        ...
        if (configContents[script.name]?.contains('%%FONT_PATH%%') ?? false) {
          tags.writeln(
    ...
    for (final script in webViewCustomScripts.where(
      ...
      // Stub webkit message handlers that the native layer injects in the real webview.
      ...
      tags.writeln(
        '$indent<script src="$scriptsBase/${script.file}"></script>',
      );
    }
